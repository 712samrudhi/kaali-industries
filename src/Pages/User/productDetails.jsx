  ...
  useEffect(() => {
    ...
    axios.get(`${BASE_URL}/product-details/${id}`)
      .then((res) => {
        ...
        // Parse customSections for each language
    ...
    <div style={{ padding: "10px 30px", background: "#f5f5f5" }}>
      <div style={{ display: "flex", gap: "5px", background: "#fff", padding: "20px", borderRadius: "10px", alignItems: "flex-start" }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          {about && (<><h3>{t.about}</h3><p>{about}</p></>)}
          {specification && (<><h3>{t.specification}</h3><p>{specification}</p></>)}
          {keyBenefits && (<><h3>{t.keyBenefits}</h3><p>{keyBenefits}</p></>)}
          {modeOfAction && (<><h3>{t.modeOfAction}</h3><p>{modeOfAction}</p></>)}
          {recommendedApplication && (<><h3>{t.recommendedApplication}</h3><p>{recommendedApplication}</p></>)}
          {suitableCrops && (<><h3>{t.suitableCrops}</h3><p>{suitableCrops}</p></>)}
          {features && (<><h3>{t.features}</h3><p>{features}</p></>)}

          {/* Custom sections (for products like seeds that use a dynamic JSON structure) */}
          ...
          <br /><br />
          <button onClick={handleBuyNow} style={{ background: "orange", border: "none", padding: "12px 25px", cursor: "pointer" }}>{t.buyNow}</button>
          <button onClick={handleAddToCart} style={{ marginLeft: "10px", background: "black", color: "#fff", border: "none", padding: "12px 25px", cursor: "pointer" }}>{t.addToCart}</button>
        </div>
      </div>
