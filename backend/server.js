...
app.use(cors({
    origin: function(origin, callback) {
        // Postman/curl/server-to-server requests (no origin) ko allow karo
...
app.post("/register", (req, res) => {
    ...
    const sql = `
        INSERT INTO farmers (name, email, mobile, address, username, password)
        VALUES (?, ?, ?, ?, ?, ?)
    `;
...
app.get("/related-products/:id", (req, res) => {
    ...
    db.query("SELECT category FROM products WHERE id = ?", [id], (err, result) => {
        ...
        db.query(
            "SELECT * FROM products WHERE category = ? AND id != ?", [category, id],
            (err2, result2) => {
...
app.delete("/product-details/:id", (req, res) => {
    ...
    db.query(
        "DELETE FROM product_details WHERE id = ?", [req.params.id],
        (err) => {
...
app.post("/product-details", detailsUpload, (req, res) => {
    ...
    const sql = `
            INSERT INTO product_details(
                ...
                suitableCrops, features, variants,
...
app.post("/feedback", (req, res) => {
    ...
    const sql =
        "INSERT INTO feedback(name, phone, rating, feedback) VALUES (?, ?, ?, ?)";

    db.query(
        sql, [name, phone, rating, feedback],
        (err, result) => {
            if (err) {
